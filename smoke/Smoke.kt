...
fun main(): Unit = runBlocking {
	...
	expectOk("city", { parse.city("charlotte") { this.country = "US" } }) {
		when {
	...
	expectOk("citySearch", { parse.citySearch("char") { this.country = "US"; this.limit = 5 } }) { if (it.cities.isNotEmpty()) null else "no results" }
	expectOk("cityNearest", { parse.cityNearest(35.2271, -80.8431) }) { if (it.distance >= 0) null else "no distance" }
	expectOk("postal", { parse.postal("28202") { this.country = "US" } }) { if (it.city == "Charlotte") null else "wrong city" }
	expectOk("postalNearby", { parse.postalNearby("28202") { this.country = "US"; this.radius = 40.0 } }) { if (it.nearby.isNotEmpty()) null else "no nearby" }
	...
	expectOk("npi", { parse.provider("1881018208") }) {
		if (it.valid && it.registered == true) null else "not registered"
	}
	expectOk("npi junk", { parse.provider("hello") }) { if (!it.valid) null else "expected invalid" }
	...
	expectOk("dns", { parse.dns("example.com") { type = "A" } }) { if (it.records.any { record -> record.type == "A" }) null else "no A records" }
	...
	expectOk("emojiSearch", { parse.emojiSearch("fire") { this.limit = 5 } }) { if (it.emojis.isNotEmpty()) null else "no results" }

	// Deep triad: asked on a free-deep endpoint always yields an object.
	expectOk("point deep triad", { parse.point(36.0726, -79.792) { this.deep = true } }) { if (it.deep != null) null else "deep missing" }

	// Honest 404 and auth errors.
	expectError("honest 404", { parse.city("notarealcityxyz") }, "not_found")
	expectError("bogus key 401", { ParseAPI("parse_bogusbogusbogus0") { this.retries = 0 }.country("US") }, "invalid_api_key")
	...
	if (appKey != null && goodAppId != null) {
		...
		val wrongId = ParseAPI(appKey) { this.appId = "com.wrong.app"; this.retries = 0 }
		...
		expectOk("app key email deep locked", { app.email("hello@gmail.com") { this.deep = true } }) {
