...
class NAICSTest {
 @Test fun hierarchyAndSearch() = runBlocking {
  ...
  val search = c.industrySearch("coffee & tea") { limit = 5 }
  assertEquals(2022, search.year)
  assertTrue(search.results.isEmpty())
  assertEquals("https://api.parseapi.com/industry/31-33", stub.requests[0].url)
  assertEquals("https://api.parseapi.com/industry?q=coffee%20%26%20tea&limit=5", stub.requests[1].url)
 }
}
...
class NAICSEvidenceTest {
 @Test fun exclusionsAndMatchRemainCompatible() = runBlocking {
  ...
  val search = client.industrySearch("sofware")
  val results: List<IndustrySearchResult> = search.results
